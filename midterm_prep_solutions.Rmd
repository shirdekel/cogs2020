...
## involved. Because of that, I will not ask you to compute
## CI estimates of a parameter from a discrete distribution.
...
Consider an experiment in which participants attempt to toss a ping pong ball as
accurately as possible to a target. Suppose that data is collected from `r n`
participants and that the mean reach error per participant is given by
...
ci_width <- ( t_crit_upper - t_crit_upper ) * sig_x_bar
ci_lower <- x_bar_obs - ci_width / 2
ci_upper <- x_bar_obs + ci_width / 2
...
- Compute power against the alternative $H_1$ that $\mu_x$ is 2
...
typeII <- pt(t_crit_upper-2, n-1, lower.tail=TRUE)
power <- pt(t_crit_upper-2, n-1, lower.tail=FALSE)
