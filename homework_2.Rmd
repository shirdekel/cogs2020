...
$\sum_{i=1}^{10}xP(x)$. Store your result in a variable
named `ans_4c`.

**(d)** It turns out it can be proven that if $X$ ~
geometric with probability of success of $p$, then
$E(X)=\frac{1}{p}$. This formula will yield a slightly
...
because part c only considered x in the range 1 to 10, yet
the true range of a geometric random variable is 1 to
infinity.
