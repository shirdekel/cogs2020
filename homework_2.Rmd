...
should have length `1` with columns named `obs_mean` and
`obs_sd`.
...
`:=` operator to add a column of zeros to `d5a_copy`. Store
the resulting `data.table` in a variable named `ans_5a`. Be
very careful that the code you write and execute here does
not modify `d5a` or you will be marked incorrect.
...
of mean values `mean_t2c`. Make sure that your resulting
`data.table` has only `cnd`, `sub` and `t2c` columns and
nothing more (no columns named `V1` etc). Also make sure
