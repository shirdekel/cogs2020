...
* In the context of two-tailed hypothesis tests, define and
...
* Be able to perform two-tailed binomial tests and t-tests
...
TODO: PULL FROM PREVIOUS LECTURE EXAMPLES

- In the previous example, what if instead of testing the hypothesis that
  $\mu_X$ is greater than $.15$, we wanted to test the hypothesis that $\mu_X$
  is simply different than $.15$?

- As we will see in the following example, the main difference is that the
  p-value gets contributions from both the lower and upper tail of the sampling
  distribution.
...
ggplot(d, aes(x, fx)) +
  geom_line() +
  geom_vline(xintercept=x_bar_lower, linetype=2, colour='grey') +
  geom_vline(xintercept=x_bar_upper, linetype=2, colour='grey') +
...
| Fail to reject H0 | Confidence   | Type II error |
</div>
<div class = "col-md-6">

|                   | H0 True    | H1 True   |
|-------------------|------------|-----------|
| Reject H0         | $\alpha$   | $1-\beta$ |
| Fail to reject H0 | $1-\alpha$ | $\beta$   |
</div>
</div>
...
- Power can only be computed with respect to some fully specified $H1$.
