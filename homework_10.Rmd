...
dd <- d[, 
        .(acc_mean = mean(acc), acc_err = sd(acc)/sqrt(.N)), 
        .(subject, trial_type)]
...
ddd <- dd[, 
         .(acc_mean = mean(acc_mean), acc_err = sd(acc_mean)/sqrt(.N)),
         .(trial_type)]
...
##
##
## Ah, crap again. Is trial type a within-subjects factor or
## a between-subjects factor?  It's easy to see that it is a
## within-subjects factor. This means that it was not
...
## Does letterColor itself have an effect on response time?
## E.g., bull fighters use red flags to antagonise bulls...
## perhaps that will make people faster? Perhaps other
## colours have a similar or different effect?

## How many colours are there?
d[, unique(letterColor)]

## Which colours belong to which colour set?
d[, unique(letterColor), .(colorSet)]

## Which factors are within and which are between?
ddesigntype <- d[, unique(subject), .(letterColor, colorSet)]
ggplot(data=ddesigntype, aes(x=letterColor, y=V1, colour=colorSet)) +
  geom_point()

## We can see:
## blue, green, and red belong to colorSet I 
## Orange, purple, and yellow belong to colorSet II 
## letterColor is within-subject
## colorSet is between-subject
...
dd <- d[, 
        .(rt_mean = mean(resp.rt), 
          acc_mean = mean(acc)), 
        .(subject, letterColor, colorSet)]

ggplot(data=dd, aes(x=letterColor, y=rt_mean, colour=colorSet)) +
  geom_violin() + 
  geom_point()

ddd <- dd[, 
         .(rt_mean = mean(rt_mean), rt_err = sd(rt_mean)/sqrt(.N)),
         .(letterColor, colorSet)]

ggplot(data=ddd, aes(x=letterColor, y=rt_mean, colour=colorSet)) +
  geom_pointrange(aes(ymin=rt_mean-rt_err, ymax=rt_mean+rt_err))
