...
d <- data.table(x,
                fx,
                region=factor(x<=xbarobs, labels=c('x > xbarobs', 'x <= xbarobs')),
                cregion=factor(x<=xbarcrit, labels=c('x > xbarcrit', 'x <= xbarcrit')))
...
g1 <- ggplot(d, aes(x, fx, colour=region)) +
  ...
  annotate('text', x=xbarobs-0.1, y=dnorm(xbarobs, muxbar, sigxbar),
           label='xobs or \n more extreme',
           colour='#00AFBB',
           hjust=1) +
  ylab('f(xbar)') +
...
g2 <- ggplot(d, aes(x, fx, colour=cregion)) +
  geom_line() +
  geom_vline(xintercept=xbarobs, colour='black', linetype=2) +
  geom_vline(xintercept=xbarcrit, colour='black', linetype=2) +
  geom_ribbon(aes(ymin=0, ymax=dnorm(x, muxbar, sigxbar), fill=cregion), alpha=0.5) +
  annotate('text', x=xbarcrit-0.1, y=dnorm(xbarcrit, muxbar, sigxbar),
...
extraction <- c(0.1460184, 0.1650378, 0.1577203, 0.1396865, 0.1731472,
                0.1713292, 0.1823381, 0.1505338, 0.1726549, 0.1522668)

n <- length(extraction)

mu_x <- 0.15 ## given H0
sigma_x <- 0.02 ## for now, just assume that we know this

## Our standard method using the information given
mu_x_bar <- mu_x
sigma_x_bar <- sigma_x / sqrt(n)
x_bar_obs <- mean(extraction)
px <- pnorm(x_bar_obs, mu_x_bar, sigma_x_bar, lower.tail=TRUE)

## Using z-test
z_obs <- (x_bar_obs - mu_x_bar) / sigma_x_bar
...
x <- seq(mu_x_bar - 5*sigma_x_bar, mu_x_bar + 5*sigma_x_bar, 0.0001)
fx <- dnorm(x, mu_x_bar, sigma_x_bar)

z <- seq(0 - 5*1, 0 + 5*1, 0.0001)
fz <- dnorm(z, 0, 1)
...
## We can see that the observed values and full distributions get transformed in
## proportion, so we get the same p-values
...
g1 <- ggplot(d, aes(t, ft, colour=region)) +
  ...
  ylab('f(t)') +
  xlab('t') +
  ggtitle('p-value value approach') +
  theme(legend.position="none")

g2 <- ggplot(d, aes(t, ft, colour=region)) +
  geom_line() +
  geom_line(aes(z, fz), colour='black') +
  geom_vline(xintercept=tcrit, colour='black', linetype=2) +
  geom_ribbon(aes(ymin=0, ymax=dt(t, n-1), fill=region), alpha=0.5) +
  annotate('text', x=-3, y=0.1,
           label='tcrit \n (rejection region)',
           colour='#00AFBB') +
