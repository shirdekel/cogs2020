...
d <- rbindlist(list(d1, d2, d3, d4, d5, dm, df), use.names=T, fill=T)
...
# dd[, f := ifelse(is.na(f), 0, m)]

dd[, total_score := sum(0.1 * 1, # quizzes (assuming you get these)
                    0.3 * mean(c(hw1, hw2, hw3, hw4, hw5)), # hw
                    0.3 * m, # midterm
                    0.3 * f, na.rm=F) # final
   , .(student_id)]
dd[, mark_class := "NA"]
...
ggplot(dd, aes(total_score, fill=mark_class)) +
  geom_histogram(breaks=seq(0.0, 1.0, 0.05), closed='left') +
  xlim(0.3, 1.0)
